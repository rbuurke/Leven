...
if __name__ == '__main__':
    ...
    full_mat = np.loadtxt('aggregate_matrix_full.txt', delimiter='\t')

    ''' PMI weighted variant '''
    df = pd.read_csv('GTRP_long.txt', sep='\t')
    ...
    for sample_size in range(1, 101):
        ...
        with open(str("aggregate_matrix_sample_" + str(sample_size) + ".txt"), 'w') as file:
