...
if __name__ == '__main__':
    ...
    # df = pd.read_csv('transcription_files - DiaReg_jv_stripped_diacritics_priority.tsv', sep = '\t')

    # # transform to long format
    # wide_to_long.expand(df, 'diareg_jv_long.txt')


    # import DiaReg OM data
    df = pd.read_csv('diareg_jv_long.txt', sep='\t')
    ...
    df.to_csv('diareg_jv_long_fixed.txt', sep='\t', index=False)

    df = pd.read_csv('diareg_jv_long_fixed.txt', sep='\t')
    ...
    np.savetxt('full_matrix_diareg_jv.txt', result_mat, fmt='%.5f', delimiter='\t')
