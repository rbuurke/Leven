...
import warnings
warnings.filterwarnings("ignore")  # to avoid the empty array warnings (sorry)
...
def cycle_dict(computation_type, loc_ssize,
    ...
    return loc_ssize, word_ssize, pd.Series(
        vec_orig.flatten()).corr(pd.Series(vec_sample.flatten()))
...
def meta_cycle(computation_type,
    ...
    if computation_type == 'location':
        results = {cycle_dict(computation_type, loc_ssize,
                   ...
                   for word_ssize in range(5, 80, 3)
    ...
    if computation_type == 'word':
        results = {cycle_dict(computation_type, loc_ssize,
                              ...
                              map_locations, dict_of_constants,
                              list_of_variation, full_mat)
                   for loc_ssize in range(5, len(list_of_locs), 3)
                   for word_ssize in dict_of_constants}
    print("Cycle done")
    return results
...
if __name__ == '__main__':
    ...
    #     'threshold_output_gtrp_var_word_sub_nl.txt',
    #     sep='\t',
    #     index=False)
    ...
    df = pd.DataFrame([result
                      ...
                      columns=['loc_ssize', 'word_ssize', 'corr'])
    df.sort_values(by=['loc_ssize', 'word_ssize']).to_csv(
        'threshold_output_gtrp_var_loc_sub_nl.txt',
        sep='\t',
        index=False)
